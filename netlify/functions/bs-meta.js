...
// ════════════════════════════════════════════════════════════════════
// bs-meta · Tagesplan & Upgrade-Planer von Slesdrafts
//   GET → aktuelle Map-Rotation + Community-Meta (Siege pro Brawler & Map)
//   Antwort wird 10 Minuten im Netlify-CDN gecacht.
//   Läuft der Zeitplan (bs-sample) nicht, sammelt diese Funktion höchstens
//   einmal pro 50 Minuten selbst ein paar Matches nach.
// ════════════════════════════════════════════════════════════════════
const CACHE = { 'Content-Type': 'application/json', 'Cache-Control': 'public, max-age=120', 'Netlify-CDN-Cache-Control': 'public, s-maxage=600, stale-while-revalidate=1800' };
const NOCACHE = { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' };
async function vielleichtSammeln() {
  const grenze = new Date(Date.now() - 50 * 60e3).toISOString();
  const r = await sb('bs_state?k=eq.meta&v=lt.' + encodeURIComponent(grenze), { method: 'PATCH', body: { v: new Date().toISOString() }, headers: { Prefer: 'return=representation' } });
  if (!r || !r.length) return null;
  return metaSammeln(4200, 24, 1);
}
function kompakt(rows) {
  const m = {};
  (rows || []).forEach(x => {
    const e = m[x.map_id] || (m[x.map_id] = { map: x.map, mode: x.mode });
    if (x.map && !e.map) e.map = x.map; if (x.mode && !e.mode) e.mode = x.mode;
    const t = e[x.typ] || (e[x.typ] = { n: 0, b: {} });
    if (x.brawler === '#') t.n = x.n; else t.b[x.brawler] = [x.wins, x.n];
  });
  return m;
}
exports.handler = async () => {
  if (!BS_KEY) return { statusCode: 501, headers: NOCACHE, body: JSON.stringify({ error: 'no-key' }) };
  const info = {};
  let events = [];
  try {
    const rot = await bs('/events/rotation', 6000);
    events = (Array.isArray(rot) ? rot : (rot.items || [])).map(e => {
      const ev = e.event || {};
      return { start: zeit(e.startTime), end: zeit(e.endTime), slot: e.slotId, id: ev.id, mode: ev.mode, map: ev.map, mods: ev.modifiers && ev.modifiers.length ? ev.modifiers : undefined };
    }).filter(e => e.id && e.map && e.end);
  } catch (e) { info.rotErr = String(e.message || e).slice(0, 60); }
  let meta = {};
  if (SB_KEY) {
    try { const s = await vielleichtSammeln(); if (s) info.sample = s; } catch (e) { info.sampleErr = String(e.message || e).slice(0, 80); }
    try { meta = kompakt(await sb('rpc/bs_meta_lesen', { method: 'POST', body: { ids: events.map(e => e.id), tage: 14 } })); }
    catch (e) { info.metaErr = String(e.message || e).slice(0, 80); }
  }
  const ok = events.length > 0 && !info.metaErr;
  return { statusCode: 200, headers: ok ? CACHE : NOCACHE, body: JSON.stringify({ t: new Date().toISOString(), events, meta, info }) };
};
