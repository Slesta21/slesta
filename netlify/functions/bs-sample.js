...
// ════════════════════════════════════════════════════════════════════
// bs-sample · läuft automatisch alle 30 Minuten (Zeitplan in netlify.toml)
// Holt die Battlelogs von ~150 Top-Spielern (weltweit + wechselnde Länder)
// und zählt Siege pro Brawler & Map für die Community-Meta.
// ════════════════════════════════════════════════════════════════════
exports.handler = async () => {
  if (!BS_KEY || !SB_KEY) return { statusCode: 200, body: 'not configured' };
  try { await sb('bs_state?on_conflict=k', { method: 'POST', body: [{ k: 'meta', v: new Date().toISOString() }], headers: { Prefer: 'resolution=merge-duplicates,return=minimal' } }); } catch (e) {}
  const r = await metaSammeln(21000, 150, 2);
  try { await metaAufraeumen(); } catch (e) { r.cleanErr = String(e.message).slice(0, 80); }
